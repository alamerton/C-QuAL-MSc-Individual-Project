...
def get_question_categories(df: pd.DataFrame):
    ...
    return treatment, assessment, diagnosis, problem_complication, \
    abnormality, etiology, medical_history
...
def get_question_types(df: pd.DataFrame):
    ...
    type
    """
    yes_no_maybe = df['Type'].str.count('')
    temporal
    factual
    diagnostic
    treatment
...
def get_dataset_statistics(dataset_path):
    ...
    question_types = get_question_types(dataset)
